$(document).ready(function () {
  ...
  $('#btnAuthorizePersonal').click(function() {
    if($("#authorizedUser").val() != '' && $("#authorizedPettyCash").val() != ''){
    $obj =  new Object();
    $obj.userOwner = $("#owner").val();
    $obj.userAuthorized = $("#authorizedUser").val();
    $obj.pettyCash = $("#authorizedPettyCash").val();
    $.ajax({
      headers: {
        'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')
      },
      type: "POST",
      url: "PettyCash/authorizePersonal",
      data: {obj:$obj},
      success: function (res) {
        if(res == 'success'){
          Swal.fire({
            position: 'center',
            type: 'success',
            title: 'El usuario se ha autorizado correctamente.',
            showConfirmButton: false,
            timer: 1000
          });
          ...
          Swal.fire({
            icon: 'error',
            title: 'Oops...',
            text: 'No se pudo hacer la operación correctamente!'
          })
        }else if(res == 'duplicate'){
          alertify.warning('El usuario que intenta authorizar ya ha sido asignado a esta caja');
        }
        },
      error:function(jqXHR, textStatus, errorThrown){
        if (jqXHR.status === 0) {
    ...
    alertify.error('Error no se han elegido los datos correspondientes');
  }
  });
...
$.ajax({
  ...
  success: function (response) {
    $('#authorizedPettyCash').html(response).fadeIn();
  }
});
...
$.ajax({
  ...
  type: "POST",
  url: "PettyCash/getAuthorizedUsers",
  success: function (response) {
    $('#authorizedUser').html(response).fadeIn();
  }
});
